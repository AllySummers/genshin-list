...
export const SelectedFilters = ({
  ...
  const filterSets = Object.values(attrFilter) as Array<Set<string>>;
    ...
    !isEmpty && (
      ...
      >
        <DragArea>
          <div className="flex h-full items-center gap-1.5 p-1 pr-4">
            {filterSets.map((filterSet) =>
              Array.from(filterSet).map((filter) => <FilterIndicator key={filter} filter={filter} />)
            )}
            <EndPadding />
          </div>
        </DragArea>
        <div className="relative ml-auto flex p-0.5">
          <BlurEffect />
          <Button onClick={clearFilters} variant="dark" size="xsmall" className="h-full pl-1">
