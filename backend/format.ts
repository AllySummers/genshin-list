...
export function formatCharacterFilter(character: Character) {
  return pick(character, 'id', 'name', 'version', 'element', 'weapon', 'region', 'rarity', 'icon');
}

function formatActive(category: ActiveCategory, active: TalentDB['combat1'], icon: string) {
  return {
    category: category,
    name: active.name,
...
export function formatActives(talents: TalentDB) {
  ...
  if (talents.combatsp !== undefined) {
    ...
    actives.push(formatActive('Alternate Sprint', talents.combatsp, talents.images.filename_combatsp!));
  }
...
function formatConstellation(constellation: ConstellationDB['c1'], icon: string) {
  return {
...
function formatCombatAttributes(attributes: CombatAttribute, category: ActiveCategory) {
  const maxLvlMap = {
...
function formatTalentParams(param: string, parameters: CombatAttribute['parameters'], talentlevel: number) {
  // replaces {paramNum:format} with formatted number
  return param.replace(/{(?<paramNum>[^}]+):(?<format>[^}]+)}/g, (_match, ...args) => {
